...
! function(window, undefined) {
	...
	var layer = {
		...
		msg: function(content, options, end) {
			var type = typeof options === 'function',
				rtheme = ready.config.theme;
			var theme = (rtheme ? rtheme + ' ' + rtheme + '-msg' : '') || 'layui-layer-msg';
			...
			}, (type && !ready.config.theme) ? {
				theme: theme + ' layui-layer-hui',
				anim: anim
			} : function() {
				options = options || {};
				if (options.icon === -1 || options.icon === undefined && !ready.config.theme) {
					options.theme = theme + ' ' + (options.theme || 'layui-layer-hui')
				}
				return options
			}()))
		},
				...
				(config.type == 2 || config.type == 4) ? function() {
					...
					if (!content.parents('.' + doms[0])[0]) {
						if (typeof content === 'object') {
							qadminIframe = content.context;
							content.after('<div id="qadmin-' + doms[0] + that.index + '"></div>')
						}
		...
		var WRAP = 'layui-layer-wrap',
			remove = function() {
				if (type === ready.type[1] && layero.attr('conType') === 'object') {
					...
					if (qadminIframe) {
						$(qadminIframe).find('#qadmin-' + doms[0] + index).before(wrap.css('display', wrap.data('display')).removeClass(WRAP))
					} else {
						wrap.css('display', wrap.data('display')).removeClass(WRAP)
					}
					$('#qadmin-' + doms[0] + index).remove()
				} else {
					if (type === ready.type[2]) {
						try {
	...
	var cache = layer.cache || {},
		theme = function(type) {
			return (cache.theme ? (' ' + cache.theme + ' ' + cache.theme + '-' + type) : '')
		};
	layer.prompt = function(options, yes) {
		...
		return layer.open($.extend({
			...
			btn: ['确定', '取消'],
			content: content,
			theme: 'layui-layer-prompt' + theme('prompt'),
			maxWidth: win.width(),
	...
	layer.tab = function(options) {
		...
		return layer.open($.extend({
			type: 1,
			theme: 'layui-layer-tab' + theme('tab'),
			resize: false,
			title: function() {
				var len = tab.length,
					ii = 1,
	...
	layer.photos = function(options, loop, key) {
		...
		loadImage(data[start].src, function(img) {
			...
			dict.index = layer.open($.extend({
				...
				moveOut: true,
				isOutAnim: false,
				theme: 'layui-layer-photos' + theme('photos'),
